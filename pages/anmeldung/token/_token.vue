  ...
  v-container(class="fill-height")
    v-row(no-gutters justify="center")
      v-col(cols="12" sm="6" md="12")
        v-row
          v-col
            h1(class="text-center py-4") {{statusMessage}}
            v-progress-linear(class="hidden-sm-and-down" :value="loading ? 33 : 66" height="12" striped)
        v-row
          v-col(cols="12" md="4" id="1")
            v-card(color="success" tile)
              v-responsive(aspect-ratio="1")
                div(class="d-flex flex-column justify-space-between align-center fill-height")
                  v-card-title(class="text-h5 font-weight-black justify-center white--text") 1. Schritt
                  div(class="absolute-flex-centered")
                    v-avatar(:max-height="avatarMaxSize" :max-width="avatarMaxSize" size="128" color="rgba(255,255,255,0.16)")
                      v-icon(color="white" size="92") mdi-check-bold
                  v-card-subtitle(class="text-subtitle-1 font-weight-medium") Anmeldedaten abgeschickt

          //- TODO: Warteliste
          v-col(cols="12" md="4" id="2")
            v-card(:loading="loading" tile :color="step2Color")
              v-responsive(aspect-ratio="1")
                template(slot="progress")
                  v-progress-linear
                div(class="d-flex flex-column justify-space-between align-center fill-height")
                  v-card-title(class="text-h5 font-weight-black justify-center" :class="{'white--text': !loading}") 2. Schritt
                  div(class="absolute-flex-centered")
                    v-avatar(v-if="!loading" :max-height="avatarMaxSize" :max-width="avatarMaxSize" size="128" color="rgba(255,255,255,0.16)")
                      //- TODO: Warteliste
                      v-icon(color="white" size="92") mdi-check-bold
                  v-card-subtitle(class="text-subtitle-1 font-weight-medium") {{step2Text}}

          v-col(cols="12" md="4" id="3")
 ...
 setup(_, ctx) {
  ...
  const statusMessage = computed(() => {
   ...
   const avatarMaxSize = computed(() => {
     return ctx.root.$vuetify.breakpoint.xsOnly ? "calc(100vw * .75)" : "calc(100vw * .2)"
   });

   const step2Text = computed(() => loading.value ? "Anmeldung wird bestätigt..." : "Anmeldung bestätigt")
